...
name=`basename $data`
$cmd JOB=1:$nj $logdir/make_delta_feats.JOB.log \
  copy-feats "$feats" \
  ark,scp:$feadir/feats_delta_$name.JOB.ark,$feadir/feats_delta_$name.JOB.scp || exit 1;

# Merge the scp,
for n in $(seq 1 $nj); do
  cat $feadir/feats_delta_$name.$n.scp 
done > $data/feats.scp
