...
# the argument is the timit directory
timit_path=$*
...
# change the directory of irstlm and sph2pipe to you own ---------luhui
export PATH=$PATH:/home/lh17/tools/irstlm/bin
sph2pipe=/home/lh17/tools/sph2pipe_v2.5/sph2pipe
if [ ! -x $sph2pipe ]; then
...
# or lower-cased
if [ ! -d "$timit_path"/TRAIN -o ! -d "$timit_path"/TEST ] && [ ! -d "$timit_path"/train -o ! -d "$timit_path"/test ]; then
...
if $uppercased; then
  ...
  ls -d "$timit_path"/TRAIN/DR*/* | sed -e "s:^.*/::" > $tmpdir/train_spk
else
...
for x in train dev test; do
  ...
  find "$timit_path"/{$train_dir,$test_dir} -not \( -iname 'SA*' \) -iname '*.WAV' \
  ...
  find "$timit_path"/{$train_dir,$test_dir} -not \( -iname 'SA*' \) -iname '*.PHN' \
